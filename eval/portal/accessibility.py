...
async def enable_accessibility_service(
    device_serial: str, adb_path: str = "adb", disable_first: bool = False
):
    ...
    logger.info(f"Configuring accessibility service: {PORTAL_SERVICE_NAME}")
    ...
    try:
        ...
        async def run_adb_command(cmd_args, step_name):
            ...
            if process.returncode != 0:
                logger.error(
                    f"Failed to {step_name}. Return code: {process.returncode}"
                )
                if stdout_str:
        ...
        if disable_first:
            ...
            if not await run_adb_command(
                disable_cmd, "disable all accessibility services"
            ):
                logger.warning(
                    "Could not disable accessibility services, proceeding anyway..."
                )
            else:
        ...
        if not await run_adb_command(
            enable_service_cmd, f"enable service {PORTAL_SERVICE_NAME}"
        ):
        ...
        if not await run_adb_command(
            enable_global_cmd, "enable accessibility globally"
        ):
            return False  # Critical failure
        ...
        if not await run_adb_command(
            disable_overlay_cmd, "disable box rendering overlay"
        ):
            logger.warning(
                "Could not disable box rendering overlay, proceeding anyway..."
            )

        logger.info(
            f"Successfully configured accessibility service: {PORTAL_SERVICE_NAME}"
        )
        return True
