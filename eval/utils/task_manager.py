...
async def initialize_task(env, task_instance: task_eval.TaskEval) -> bool:
    ...
    try:
        ...
        if hasattr(task_instance, "initialize_task") and callable(
            getattr(task_instance, "initialize_task")
        ):
        ...
        elif hasattr(task_instance, "setup") and callable(
            getattr(task_instance, "setup")
        ):
...
def check_task_success(env, task_instance: task_eval.TaskEval) -> bool:
    ...
    try:
        ...
        if hasattr(task_instance, "check_success") and callable(
            getattr(task_instance, "check_success")
        ):
            return task_instance.check_success(env)
        # Fall back to is_successful() which is common in some AndroidWorld tasks
        elif hasattr(task_instance, "is_successful") and callable(
            getattr(task_instance, "is_successful")
        ):
            return task_instance.is_successful(env)
        else:
            # If neither is available, mark as failed
            logger.warning(
                f"Task {task_name} has no check_success() or is_successful() method"
            )
            return False
    except Exception as e:
...
def teardown_task(env, task_instance: task_eval.TaskEval) -> bool:
    ...
    try:
        if hasattr(task_instance, "tear_down") and callable(
            getattr(task_instance, "tear_down")
        ):
            task_instance.tear_down(env)
            logger.info(f"Task {task_name} torn down using tear_down() method")
            return True
        elif hasattr(task_instance, "teardown") and callable(
            getattr(task_instance, "teardown")
        ):
